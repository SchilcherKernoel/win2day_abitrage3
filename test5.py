...
def _get_odds():
    ...
    for event_id in event_id_list:
        ...
        for item in bets:
            bet_type_list, bet_label, bet_odd = [], [], []

            bet_type = [item.css_first("div.t3-match-details__entry-header").text()]
            bet_type_list.append(_string_list_clean(bet_type))

            label = item.css("div.t3-bet-element__label")
            for lbl in label:
                bet_label.append(_string_list_clean([lbl.text()]))

            bet = item.css("div.t3-bet-element__field")
            for bt in bet:
                bet_odd.append(_string_list_clean([bt.text()]))

            _odd_handler_tipp3(bet_type, bet_label, bet_odd)
